...
int main(int argc, char** argv) {
    ...
    float xs=1.0; float weight=1.0; float luminosity=59500;
    ...
    arbre->SetBranchAddress("againstElectronVLooseMVA62018_2",&againstElectronVLooseMVA62018_2);
    arbre->SetBranchAddress("againstElectronLooseMVA62018_2",&againstElectronLooseMVA62018_2);
    ...
    arbre->SetBranchAddress("againstElectronVLooseMVA62018_1",&againstElectronVLooseMVA62018_1);
    arbre->SetBranchAddress("againstElectronLooseMVA62018_1",&againstElectronLooseMVA62018_1);
    arbre->SetBranchAddress("againstMuonTight3_1",&againstMuonTight3_1);
    arbre->SetBranchAddress("byLooseDeep_1",&byLooseDeep_1);
    arbre->SetBranchAddress("byMediumDeep_1",&byMediumDeep_1);
    arbre->SetBranchAddress("byTightDeep_1",&byTightDeep_1);
    arbre->SetBranchAddress("byVTightDeep_1",&byVTightDeep_1);
    arbre->SetBranchAddress("byTightDPF_1",&byTightDPF_1);
   ...
   float bins_mtt0[] = {0,40,60,80,100,120,140,160,180,200,220,240,260,280,300,320,340};
   ...
   TauTriggerSFs2017* ttsf=new TauTriggerSFs2017("TauAnalysisTools/TauTriggerSFs/data/tauTriggerEfficiencies2017.root","ditau", "2017", "tight", "MVAv2");
   ...
   for (Int_t i = 0; i < nentries_wtn; i++) {
	...
	if (do_rivet){
        ...
        bool trigger1=(passDoubleTightTau35TightID && pt_1>40 && pt_2>40);
        bool trigger2=(passDoubleMediumTau40TightID && pt_1>45 && pt_2>45);
        bool trigger3=(passDoubleTightTau40ID && pt_1>45 && pt_2>45);
        bool trigger4=(passDoubleMediumHPSTau35 && pt_1>40 && pt_2>40);

        if (sample!="data_obs" && !trigger4) continue;
        if (sample=="data_obs" && run<317509 && !trigger1 && !trigger2 && !trigger3) continue;
        if (sample=="data_obs" && run>=317509 && !trigger4) continue;
	...
	float signalRegion=(byVLooseIsolationMVArun2v2DBoldDMwLT_1 && byVLooseIsolationMVArun2v2DBoldDMwLT_2 && byMediumDeep_1 && byMediumDeep_2);
        float antiisoRegion1=(byVLooseIsolationMVArun2v2DBoldDMwLT_1 && !byMediumDeep_1 && byMediumDeep_2 && byVLooseIsolationMVArun2v2DBoldDMwLT_2);
        float antiisoRegion2=(byVLooseIsolationMVArun2v2DBoldDMwLT_2 && !byMediumDeep_2 && byMediumDeep_1 && byVLooseIsolationMVArun2v2DBoldDMwLT_1);

	if (!againstElectronVLooseMVA6_2 or !againstElectronVLooseMVA6_1 or !againstMuonLoose3_1 or !againstMuonLoose3_2) continue;
	...
	mytau2.SetPtEtaPhiM(pt_2,eta_2,phi_2,m_2);
        ...
        if (sample=="W"){
            weight=62.95;
            if (numGenJets==1) weight=17.95;
            else if (numGenJets==2) weight=4.56;
            else if (numGenJets==3) weight=3.14;
            else if (numGenJets==4) weight=3.25;
        }
	...
	if (sample!="embedded" && sample!="data_obs" && gen_match_1==5) aweight=aweight*0.89; // Tau ID SF
        ...
        if (sample!="embedded" && sample!="data_obs" && gen_match_1==5) aweight=aweight*ttsf->getTriggerScaleFactor(mytau1.Pt(), mytau1.Eta(), mytau1.Phi(), l1_decayMode); // Tau trigger SF FIXME
        if (sample!="embedded" && sample!="data_obs" && gen_match_2==5) aweight=aweight*ttsf->getTriggerScaleFactor(mytau2.Pt(), mytau2.Eta(), mytau2.Phi(), l2_decayMode); // Tau trigger SF FIXME
...
cout<<ttsf->getTriggerScaleFactor(mytau2.Pt(), mytau2.Eta(), mytau2.Phi(), l2_decayMode)<<" "<<ttsf->getTriggerScaleFactorUncert(mytau2.Pt(), mytau2.Eta(), mytau2.Phi(), l2_decayMode, "Down")<<" "<<ttsf->getTriggerScaleFactorUncert(mytau2.Pt(), mytau2.Eta(), mytau2.Phi(), l2_decayMode, "Up")<<endl;;
...
//cout<<mytau1.Pt()<<" "<<w->function("tt_PFTau35OR40_tight_kit_data")->getVal()<<" "<<w->function("tt_PFTau35OR40_tight_kit_embed")->getVal()<<" "<<w->function("tt_emb_PFTau35OR40_tight_kit_ratio")->getVal()<<endl;
	...
	for (int k=0; k<nbhist; ++k){
	   ...
	   // Check again the pT after the shifts //FIXME

	   if (mytau1.Pt()<50 && mytau2.Pt()<50) continue;
           if (mytau1.Pt()<40 or mytau2.Pt()<40) continue; //FIXME
	
	   myvar=mtt;
	   //myvar=(mytau1+mytau2).M();
