...
TF1 *M_FR(int WP, std::string type, std::string files, std::string num, std::string denum, std::string name, TH2F * hist2D_lep, Double_t fMin, Double_t fMax) {
   ...
   if (type.find("Constant") < 140){
    ...
    canvas->SetTitle("");
    canvas->SetGrid();
    TGraph_FR->GetYaxis()->SetRangeUser(0.5,3.5);
    ...
    TFile *FR_H = new TFile("osss_em_2018.root", "UPDATE");
    FR_H->cd();
...
void Fit_QCDSF_em() {
    ...
    TF1* m11 = M_FR(1, "Line2Par2", "qcdfiles_em/Sub.root", "INOS0", "INSS0", "OSSS_qcd_0jet_2018", Fit_Value_tau, fMin, fMax);
    TF1* m111 = M_FR(11, "Line2Par", "qcdfiles_em/Sub.root", "INOS0", "INSS0", "OSSS_qcd_0jet_2018_par1", Fit_Value_tau, fMin, fMax);
    TF1* m112 = M_FR(12, "Constant", "qcdfiles_em/Sub.root", "INOS0", "INSS0", "OSSS_qcd_0jet_2018_par0", Fit_Value_tau, fMin, fMax);

    TF1* m12 = M_FR(2, "Line2Par", "qcdfiles_em/Sub.root", "INOS1", "INSS1", "OSSS_qcd_1jet_2018", Fit_Value_tau, fMin, fMax);
    TF1* m13 = M_FR(3, "Line2Par", "qcdfiles_em/Sub.root", "INOS2", "INSS2", "OSSS_qcd_2jet_2018", Fit_Value_tau, fMin, fMax);
