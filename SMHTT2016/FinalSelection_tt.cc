...
int main(int argc, char** argv) {
   ...
   TFile* ff_file = TFile::Open("HTTutilities/Jet2TauFakes/data/SM2016_ML/tight/tt/fakeFactors_tight.root");
   ...
   for (Int_t i = 0; i < nentries_wtn; i++) {
	...
	if (sample!="embedded" && sample!="data_obs" && gen_match_1==5) aweight=aweight*0.95; // Tau ID SF
        if (sample!="embedded" && sample!="data_obs" && gen_match_2==5) aweight=aweight*0.95; // Tau ID SF
        ...
        if (sample!="embedded" && sample!="data_obs" && gen_match_1==5){
          w2->var("t_pt")->setVal(mytau1.Pt());
          w2->var("t_eta")->setVal(mytau1.Eta());
          w2->var("t_dm")->setVal(l1_decayMode);
//cout<<mytau1.Pt()<<" "<<mytau1.Eta()<<" "<<l1_decayMode<<" "<<w2->function("t_genuine_TightIso_tt_ratio")->getVal()<<endl;
          aweight=aweight* w2->function("t_genuine_TightIso_tt_ratio")->getVal();
	}
        if (sample!="embedded" && sample!="data_obs" && gen_match_2==5){
          w2->var("t_pt")->setVal(mytau2.Pt());
          w2->var("t_eta")->setVal(mytau2.Eta());
	...
	if (sample=="qqH_htt125") aweight=aweight*1.00;//Prefiring effect
