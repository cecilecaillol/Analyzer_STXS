...
float get_ff(float pt, float mt, float mvis, int njets, float frac_tt, float frac_qcd, float frac_w, TString shift){
   ...
   // Mvis closure
   if (njets==0){
      if (shift=="mvisclosure_qcd_up" or shift=="mvisclosure_qcd_up_0jet") ff_qcd = ff_qcd*(1+2*(get_mvis_closure(mvis, mvisclosure_qcd_0jet)-1));
      else if (shift=="mvisclosure_qcd_down" or shift=="mvisclosure_qcd_down_0jet") ff_qcd = ff_qcd;
      else ff_qcd = ff_qcd*get_mvis_closure(mvis, mvisclosure_qcd_0jet);

      if (shift=="mvisclosure_w_up" or shift=="mvisclosure_w_up_0jet") ff_w = ff_w*(1+2*(get_mvis_closure(mvis, mvisclosure_w_0jet)-1));
      else if (shift=="mvisclosure_w_down" or shift=="mvisclosure_w_down_0jet") ff_w = ff_w;
      else ff_w = ff_w*get_mvis_closure(mvis, mvisclosure_w_0jet);
   }
   else if (njets==1){
      if (shift=="mvisclosure_qcd_up" or shift=="mvisclosure_qcd_up_1jet") ff_qcd = ff_qcd*(1+2*(get_mvis_closure(mvis, mvisclosure_qcd_1jet)-1));
      else if (shift=="mvisclosure_qcd_down" or shift=="mvisclosure_qcd_down_1jet") ff_qcd = ff_qcd;
      else ff_qcd = ff_qcd*get_mvis_closure(mvis, mvisclosure_qcd_1jet);

      if (shift=="mvisclosure_w_up" or shift=="mvisclosure_w_up_1jet") ff_w = ff_w*(1+2*(get_mvis_closure(mvis, mvisclosure_w_1jet)-1));
      else if (shift=="mvisclosure_w_down" or shift=="mvisclosure_w_down_1jet") ff_w = ff_w;
      else ff_w = ff_w*get_mvis_closure(mvis, mvisclosure_w_1jet);
   }
   else {
      if (shift=="mvisclosure_qcd_up" or shift=="mvisclosure_qcd_up_2jet") ff_qcd = ff_qcd*(1+2*(get_mvis_closure(mvis, mvisclosure_qcd_2jet)-1));
      else if (shift=="mvisclosure_qcd_down" or shift=="mvisclosure_qcd_down_2jet") ff_qcd = ff_qcd;
      else ff_qcd = ff_qcd*get_mvis_closure(mvis, mvisclosure_qcd_2jet);

      if (shift=="mvisclosure_w_up" or shift=="mvisclosure_w_up_2jet") ff_w = ff_w*(1+2*(get_mvis_closure(mvis, mvisclosure_w_2jet)-1));
      else if (shift=="mvisclosure_w_down" or shift=="mvisclosure_w_down_2jet") ff_w = ff_w;
