...
int main(int argc, char** argv) {
   ...
   if (sample=="data_obs" or sample=="embedded") nbhist=1+14;
      ...
      nbhist=1+42+14;
      if (is_ggh) nbhist=1+42+18;
      if (is_qqh) nbhist=1+42+20;
   }
   ...
   for (Int_t i = 0; i < nentries_wtn; i++) {
	...
	if (sample!="embedded" && sample!="data_obs"){
	  ...
	  aweight=aweight*bweight_central;
	...
	for (int k=0; k<nbhist; ++k){
	   ...
	   massJets=mjj;
           ...
           if (sample!="data_obs" && sample!="embedded"){
	        ...
	        if (norecoil){
              ...
              else if (!norecoil && k>24 && k<29){ // recoil x 12
                if (k==25){mymet.SetPtEtaPhiM(met_resolutionDown, 0, metphi_resolutionDown, 0); mtt=m_sv_ResolutionDown;}
                else if (k==26){mymet.SetPtEtaPhiM(met_resolutionUp, 0, metphi_resolutionUp, 0); mtt=m_sv_ResolutionUp;}
                else if (k==27){mymet.SetPtEtaPhiM(met_responseDown, 0, metphi_responseDown, 0); mtt=m_sv_ResponseDown;}
                else if (k==28){mymet.SetPtEtaPhiM(met_responseUp, 0, metphi_responseUp, 0); mtt=m_sv_ResponseUp;}
              }

              else if (k>28 && k<31 && (name=="TT" or sample=="TTToHadronic" or sample=="TTTo2L2Nu" or sample=="TTToSemiLeptonic")){ //Top pt reweighting x 2
                if (k==29) weight2=1.0/topfactor;
                else if (k==30) weight2=topfactor;
              }

              else if (k>30 && k<33 && (sample=="DY" or sample=="Z")){ //DY pt reweighting x 2
                if (k==31) weight2=(zptweight-0.10*(zptweight-1))/zptweight;
                else if (k==32) weight2=(zptweight+0.10*(zptweight-1))/zptweight;
              }

              if (k>32 && k<35){ // EES x 2 
                if (k==33){myele*=(pt_1_ScaleDown/myele.Pt()); mymet=mymet-myele+myrawele; mtt=m_sv_elescaleESDown;} 
                else if (k==34){myele*=(pt_1_ScaleUp/myele.Pt()); mymet=mymet-myele+myrawele; mtt=m_sv_elescaleESUp;}
              }

              if (k>34 && k<41){ // MES x 6 
                if (k==35 && fabs(mymu.Eta())<2.4 && fabs(mymu.Eta())>=2.1){mymu*=0.973; mymet=mymet+(0.027/0.973)*mymu; mtt=m_sv_muonESDown;}
                else if (k==36 && fabs(mymu.Eta())<2.4 && fabs(mymu.Eta())>=2.1){mymu*=1.027; mymet=mymet-(0.027/1.027)*mymu; mtt=m_sv_muonESUp;}
		else if (k==37 && fabs(mymu.Eta())<2.1 && fabs(mymu.Eta())>=1.2){mymu*=0.991; mymet=mymet+(0.009/0.991)*mymu; mtt=m_sv_muonESDown;}
                else if (k==38 && fabs(mymu.Eta())<2.1 && fabs(mymu.Eta())>=1.2){mymu*=1.009; mymet=mymet-(0.009/1.009)*mymu; mtt=m_sv_muonESUp;}
                else if (k==39 && fabs(mymu.Eta())<1.2){mymu*=0.996; mymet=mymet+(0.004/0.996)*mymu; mtt=m_sv_muonESDown;}
                else if (k==40 && fabs(mymu.Eta())<1.2){mymu*=1.004; mymet=mymet-(0.004/1.004)*mymu; mtt=m_sv_muonESUp;}
              }

              if (k>40 && k<43){
		if (k==41) {weight2=prefiring_weight_down/(prefiring_weight+0.000000001);}
                else if (k==42) {weight2=prefiring_weight_up/(prefiring_weight+0.000000001);}
	      }
	   ...
	   if (do_control_plots==1){
	      ...
	      if (numberJets<2) mass7=-10;
           ...
           if (sample!="embedded" && !is_ggh && !is_qqh && sample!="data_obs" && k>42 && k<57){ // QCD x 14
...
//cout<<ssweight*sscorr<<endl;
	   ...
	   if (sample=="embedded" && aweight*weight2>1) aweight=0.0; //FIXME 
           ...
           if ((do_control_plots==1 && pass_mt && pass_dzeta) or (do_control_plots==2 && pass_mt) or (numberJets==0 && pass_mt && pass_dzeta)){
           ...
           if ((pass_mt && pass_dzeta && do_control_plots==1) or (pass_dzeta && do_control_plots==2) or (pass_mt && pass_dzeta && numberJets>=2 && massJets>350)){
               if (k==0 && is_includedInEmbedded && signalRegion && q_1*q_2<0)
